...
public class TestTonlibBuilder {
  ...
  public void testTonlibBuilderWindowsIgnoreCache() {
    if (SystemUtils.IS_OS_WINDOWS) {
      Tonlib tonlib =
          Tonlib.builder().verbosityLevel(VerbosityLevel.FATAL).ignoreCache(false).build();
      assertThat(tonlib.getLast()).isNotNull();

      tonlib = Tonlib.builder().verbosityLevel(VerbosityLevel.FATAL).ignoreCache(true).build();
      assertThat(tonlib.getLast()).isNotNull();
    }
  }
  ...
  public void testTonlibBuilderUbuntuIntegratedLib() {
    if (SystemUtils.IS_OS_LINUX) {
      Tonlib tonlib =
          Tonlib.builder().pathToGlobalConfig("/mnt/testnet-global.config.json").build();
      assertThat(tonlib.getLast()).isNotNull();
    }
  }
  ...
  public void testTonlibBuilderUbuntuIntegratedConfig() {
    if (SystemUtils.IS_OS_LINUX) {
      Tonlib tonlib =
          Tonlib.builder().pathToTonlibSharedLib("/mnt/tonlibjson-linux-x86_64.so").build();
      assertThat(tonlib.getLast()).isNotNull();
    }
  }
